...
6. If the input content is empty, malformed, or missing critical sections, still produce a valid review. Score poorly (1-2) on affected dimensions and list the gaps as issues.
...
- `overallScore` is an integer 1-5 (average of dimension scores, rounded to nearest integer). Each dimension score is also an integer 1-5.
