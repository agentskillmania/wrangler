// packages/wrangler-devtool/src/tools/init-project.ts
// Project 初始化
...
export async function initProject(cwd: string, options: InitOptions): Promise<void> {
  ...
  // .gitignore（评估产物、依赖不入库）
  const gitignore = await loadTemplate('gitignore');
  await writeFile(join(dir, '.gitignore'), gitignore, 'utf-8');
}
