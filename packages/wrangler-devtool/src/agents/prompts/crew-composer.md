...
1. **Sequential Pipeline**: Agents work in order (A -> B -> C). Best for linear workflows like research -> writing -> editing.
2. **Coordinator/Dispatcher**: Primary agent routes to specialist workers. Best when different input types need different handlers.
3. **Parallel Fan-out**: Multiple agents work simultaneously, results aggregated. Best for independent analysis tasks.
4. **Hierarchical**: Primary decomposes complex goals, delegates subtasks. Best for multi-level task breakdown.
