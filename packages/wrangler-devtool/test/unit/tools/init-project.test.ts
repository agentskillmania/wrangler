...
describe('initProject', () => {
  ...
  it('should create agent project', async () => {
    const dir = join(tempDir, 'agent-project');
  ...
  it('should create crew project', async () => {
    const dir = join(tempDir, 'crew-project');
  ...
  it('should create missing directories', async () => {
    const dir = join(tempDir, 'deeply', 'nested', 'project');
    await initProject(dir, { type: 'agent' });
  ...
  it('should skip git init when already in a git repo', async () => {
    ...
    // but project files should still be created
  ...
  it('should create .gitignore with eval artifacts excluded', async () => {
    const dir = join(tempDir, 'with-gitignore');
    await initProject(dir, { type: 'agent' });

    const gitignore = readFileSync(join(dir, '.gitignore'), 'utf-8');
    expect(gitignore).toContain('.eval/');
    expect(gitignore).toContain('node_modules/');
  });

  it('should create skill project without AGENT.md or CREW.md', async () => {
    const dir = join(tempDir, 'skill-project');
