...
import { wrapToColtsTool } from '../tools/wrap-tool.js';
...
export function createTodolistSupport(deps: {
  ...
  const rawTool = createTodolistTool(deps.getList, deps.setList);
  const tool = wrapToColtsTool(rawTool as unknown as WranglerToolDef);
