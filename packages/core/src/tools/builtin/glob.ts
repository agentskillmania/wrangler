...
export function createGlobTool(deps: WorkspaceToolDeps): WranglerToolDef<typeof GlobSchema> {
  return {
    name: 'glob',
    ...
    async execute(args) {
      ...
      return { output, metadata: { pattern: args.pattern, files, total, truncated } };
    },
  };
}
