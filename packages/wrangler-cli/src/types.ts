...
export type TimelineEntry =
  ...
  | { type: 'assistant'; id: string; seq: number; content: string; timestamp: number; isStreaming?: boolean }
  | { type: 'tool'; id: string; seq: number; tool: string; summary: string; isRunning?: boolean; duration?: number; timestamp: number }
  ...
  | { type: 'subagent-card'; id: string; seq: number; agentName: string; status: 'running' | 'completed'; summary?: string; timestamp: number }
...
export function parseCommand(input: string): ParsedCommand {
  ...
  switch (cmd) {
    ...
    case '/session': return { type: 'switch-session', name: parts[1] ?? '' };
    case '/clear': return { type: 'clear' };
    case '/help': return { type: 'help' };
    default: return { type: 'message', content: input };
  }
}
