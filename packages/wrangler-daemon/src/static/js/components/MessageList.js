    ...
    <div>
      ${entries.map(function (entry, idx) {
          ...
          return html`
            <div key=${entry.id || idx} class="chat-msg chat-msg-assistant">
              <div class="chat-msg-body">
                <pre style="white-space:pre-wrap;margin:0">${esc(display)}</pre>
              </div>
            </div>
          `;
        }
        if (entry.role === 'tool') {
          ...
          var toolDisplay = toolTruncated
            ? toolContent.slice(0, maxLen) + '...[truncated]'
            : toolContent;
          var argsContent = entry.toolArguments || '';
          var argsTruncated = argsContent.length > maxLen;
          var argsDisplay = argsTruncated
            ? argsContent.slice(0, maxLen) + '...[truncated]'
            : argsContent;
          return html`
            <div
              key=${entry.id || idx}
              class="chat-msg"
              style="background:rgba(139,148,158,0.06);border-left:3px solid var(--accent-secondary)"
            >
              <div class="chat-msg-body" style="font-size:12px">
                <div style="font-weight:600;color:var(--accent-secondary)">
                  [tool] ${esc(entry.toolName || 'unknown')}
                </div>
                ${argsContent &&
                html`<pre
                  style="white-space:pre-wrap;margin:4px 0;font-size:11px;color:var(--text-muted)"
                >
${esc(argsDisplay)}</pre
                >`}
        ...
        if (entry.role === 'error') {
          return html`
            <div key=${entry.id || idx} class="chat-msg chat-msg-error">
              <div class="chat-msg-body">
                ${esc(entry.errorMessage || entry.content || 'Unknown error')}
              </div>
            </div>
          `;
        }
